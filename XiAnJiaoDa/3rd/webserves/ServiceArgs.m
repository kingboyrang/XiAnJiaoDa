...
static NSString *defaultWebServiceUrl=@"http://223.4.216.54:8888/xajdcg_webservice/userLogin";
static NSString *defaultWebServiceNameSpace=@"http://intf.gzltj.service.gwzd.com";
...
- (NSString*)operationPath{
    ...
    if (url.port) {
        return [NSString stringWithFormat:@"%@:%d",url.host,[url.port intValue]];
    }
    return [url host];
}
-(NSDictionary*)headers{
    ...
    [dic setValue:[self operationPath] forKey:@"Host"];
    if (self.httpWay==ServiceHttpPost) {
...
- (NSURLRequest*)request{
    ...
    NSMutableURLRequest *req=[NSMutableURLRequest requestWithURL:webURL];
    //头部设置
